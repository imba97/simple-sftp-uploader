...
import { Client as SSHClient, SFTPWrapper, ConnectConfig } from 'ssh2'
...
export default class SftpUploadPlugin {
   ...
   */
  public async start() {
    this._consoleMark.start = '┌'
    this._consoleMark.info = '├'
    this._consoleMark.end = '└'
   ...
   * @return
   */
  public async uploadFile(_local: string, _remote: string): Promise<null> {
    await this.mkdir(_remote)
   ...
   */
  public exists(src: string): Promise<boolean> {
   ...
   * @returns
   */
  public async deleteFiles(remote: string, exclude?: RegExp): Promise<null> {
    this._remote = remote
    ...
    // 获取文件夹内容
    const files = await this.readdir(remote)

    // 排除
    this._remoteDirFiles = exclude
      ? files.filter((filename) => !exclude.test(filename))
      : files

    // 清理
    return this.doCleardir()
  }
   ...
   * @param src 远程路径
   */
  public async readdir(src: string): Promise<string[]> {
   ...
   * @return
   */
  public async mkdir(dirPath: string): Promise<null> {
    return this.doMkdir(dirPath)
  }
   ...
   * @return
   */
  private async doMkdir(dirPath: string | string[], index = 0): Promise<null> {
    return new Promise((resolve) => {
      ;(async () => {
          ...
          this._sftp.mkdir(
            ...
            () => {
              ...
              if (index < dirs.length - 1) {
                resolve(this.doMkdir(dirs, index + 1))
              } else {
                resolve(null)
              }
            }
  ...
  private async upload(i = 0): Promise<any> {
    ...
    await this.doUpload(
      this._directoryTree.local[i],
      this._directoryTree.remote[i]
    )
   ...
   * @return
   */
  private async doUpload(_local: string, _remote: string): Promise<null> {
    return new Promise((resolve) => {
   ...
   * @return
   */
  private async doCleardir(i = 0): Promise<null> {
